...
from .spider import ASYNCIO, LinovelibMobileSpider,UAASpider  # type: ignore[attr-defined]
...
class EpubWriter:
    ...
    def _write_epub(self,
        ...
        if desc != '':
            ...
            book.add_metadata(None,'meta',desc,{'property':"desc"})
        if latest_update_chapterId != '':
            book.add_metadata(None,'meta',latest_update_chapterId,{'property':'latest_update_chapterId'})
        ...
        if updateTime !='':
            ...
            epub.write_epub((out_folder) + "/" + prefix + sanitize_pathname(title) + '.epub', book,{'mtime':datetime.strptime(updateTime, '%Y-%m-%dT%H:%M:%S.000+00:00')})
        else:
...
class Linovelib2Epub:
    ...
    def __init__(self,
        ...
        if is_uaa :
            self._spider = UAASpider(spider_settings=self.spider_settings)
        else:
    ...
    def run(self) -> None:
        ...
        if novel_pickle_path.exists():
            if self.common_settings['load_pickle']:
                self.logger.info(f'load pickle {pickle_path}')
